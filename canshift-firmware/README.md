...
esptool.py --chip esp32 -p "$PORT" -b 460800 \
  ...
  0x3B0000 "canshift-spiffs-${TAG}-crowpanel_28.bin"
```
...
- SPIFFS partition offset `0x3B0000` matches `ota_4mb_wifi.csv` (the
...
| `spiffs`      | 832 KB @ `0x310000`    | 256 KB @ `0x3B0000`        |
...
~190 KB (5 Orbitron .bin fonts ~34 KB + canonical JSON configs ~25 KB + 25
sensor icons ~77 KB + `.bak` atomic-write companions ~25 KB + ~15 % SPIFFS
overhead) — well under 256 KB with headroom for future user pushes.
...
- `canshift-studio/src/hooks/useFirmwareFlash.ts` (`0x310000` constant) —
  update to `0x3B0000` before the bundled Studio flasher can image a #1117+
  build.
