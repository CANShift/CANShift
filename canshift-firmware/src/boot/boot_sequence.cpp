...
namespace {
static lv_obj_t *s_splashTitle = nullptr;  // text placeholder shown until SD mounts
static lv_obj_t *s_splashLogo = nullptr;   // banner image, replaces title after SD mount
...
// Replace the placeholder text with the banner image once the SD is mounted.
// The .bin is generated by scripts/png_to_lvgl_bin.py and lives at
// /assets/logo_banner.bin on the SD (LVGL FS drive 'S:').
// If the file is missing, keep the text title — never leave the splash blank.
static void setSplashLogo() {
    if (!s_splashTitle)
        return;

    static const char *PATH = "S:/assets/logo_banner.bin";
    lv_fs_file_t f;
    if (lv_fs_open(&f, PATH, LV_FS_MODE_RD) != LV_FS_RES_OK) {
        LOG_WARN("BOOT", "Splash banner not found on SD (%s) — keeping text title", PATH);
        return;
    }
    lv_fs_close(&f);

    lv_obj_del(s_splashTitle);
    s_splashTitle = nullptr;
...
void BootSequence::run() {
    ...
    setSplashLogo(); // SD now mounted, swap text title for banner image
    ...
    // Hold the splash at 100% for ~1.2 s so the user actually sees the banner
    // and final state before the dashboard takes over the screen.
    delay(1200);
