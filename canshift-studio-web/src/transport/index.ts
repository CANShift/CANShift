...
// canonical). The device-config opcodes (0x03/0x04) occupy the next free slot
// pair after CMD_PUSH_CONFIG and are wired here on the studio-web side; the
// matching firmware dispatcher lands in a coordinated follow-up. Until then
// the firmware's `default` branch acks every unknown cmd with `{status:"ok"}`,
// so `put` succeeds without persistence and `get` returns no config — both
// surface as a "no_config" outcome to callers.
// ---------------------------------------------------------------------------
