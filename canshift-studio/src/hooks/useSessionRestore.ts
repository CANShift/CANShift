...
// store and, if found, opens the file directly (no dialog). Failures are
// best-effort but no longer silent — migration, validation AND file-read
// errors all surface through `useLogStore` so the user can diagnose a missing
// or moved session file from the CLI panel (audit S-L-1, #1015).
...
export function useSessionRestore(): void {
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (!result.success) {
        // File-read failure (most often: file moved or deleted since last
        // session, or a permission flip on the parent directory). Previously
        // swallowed — audit S-L-1 (#1015) calls this out as a lost
        // observability surface. We don't change the behaviour (still no UI
        // toast — the editor falls back to its default empty state), just
        // emit a warn so the CLI panel records what happened.
        const reason = result.error ?? 'unknown error'
        log('warn', `Session restore skipped — could not open ${lastPath}: ${reason}`)
        return
      }
      if (!result.content) {
        // Main-process handler returned success without content — shouldn't
        // happen in practice, but if a future IPC drift starts producing this
        // shape we want a log breadcrumb rather than another silent return.
        log('warn', `Session restore skipped — empty content for ${lastPath}`)
        return
      }
