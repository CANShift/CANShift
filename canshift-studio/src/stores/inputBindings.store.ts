// inputBindings.store.ts — Physical GPIO button bindings (issue #1015, S-H-3).
//
// Pre-refactor, `InputBindingsSection` IPC'd `inputBindingsIpc.read()` from
// inside `useEffect` and held the editable draft + save flags in local
// `useState`. That matches the "useEffect for data fetching" anti-pattern.
//
// The store now owns the load/save lifecycle. The component becomes a thin
// editor: it selects `bindings` + actions, and only schedules a single
// `load()` call when it mounts.
...
export const useInputBindingsStore = create<InputBindingsState>()((set, get) => ({
  ...
  load: async () => {
    // Already loaded — nothing to do. The editor surface is mounted once
    // inside DeviceConfigRoute and the on-disk config doesn't change behind
    // our back during a session.
    ...
    try {
      ...
      // bindings from scratch even if the on-disk read failed.
      set({ loaded: true })
    }
  },
